...
### 5. 啟動 Streamlit 實時監測與標記工具看板 (Raspberry Pi 端 或 開發機/PC 端)

Streamlit 看板提供了一個直觀的圖形化介面，方便進行系統運行狀態監控、硬體診斷、數據集標記與硬體校準。

1. **安裝看板依賴**:
    * **PC 端** (若要在 PC 預覽或測試看板，請在 PC 的虛擬環境中安裝):
      ```bash
      pip install streamlit pandas sounddevice
      ```
    * **Raspberry Pi 端** (受限於系統安全政策，建議使用 `--break-system-packages`，或在虛擬環境中安裝):
      ```bash
      sudo pip3 install streamlit pandas sounddevice --break-system-packages
      ```
2. **執行 Streamlit 服務**:
    在專案根目錄下執行：
    ```bash
    streamlit run pi_client/Streamlit.py
    ```
    啟動後，瀏覽器會自動開啟看板介面（預設網址為 `http://localhost:8501`）。

3. **五大核心功能分頁說明**:
    * **📊 垃圾桶運行監控 (Live Dashboard)**: 
      * 實時同步展示 FSM (有限狀態機) 當前狀態 (`IDLE`/`TRIGGER`/`CAPTURE`)。
      * 顯示垃圾投遞的新增重量、最後一次投遞的實體照片快照、碰撞錄音回放與 Mel-spectrogram 頻譜圖。
      * 呈現本地交叉融合推論或雲端 Gemini 備援的詳細預測結果、信心值與推論延遲。
      * 自動紀錄並滾動展示最近 50 次投遞的歷史日誌，具備 `KeyError` 等版本相容健壯性防護。
    * **🎤 麥克風收訊診斷**:
      * 提供手動錄音功能（可設定時長 1~5 秒），實時繪製聲音波形與計算 Peak / RMS 振幅以評估收音品質。
      * 若開發機或樹莓派無連接實體麥克風，系統會自動啟動「虛擬模擬麥克風」模式，產生高品質模擬碰撞訊號，確保功能可用。
    * **📁 採集 Dataset 工具 (人工標記與導出)**:
      * 實作 `index=None` 無偏見標記機制（防自動預選造成污染），提供自訂描述備註與重量紀錄。
      * 每次歸檔會自動把照片、頻譜與音檔儲存至 `dataset/{category}`，並產生包含詳細元資料的 sidecar `.json` 描述檔。
      * 顯示最新 10 筆歸檔紀錄，並設有「撤銷最近一次採集」防呆按鈕以快速刪除誤標樣本。
      * 支援即時統計各分類的影像與音軌樣本數，並能**一鍵壓縮打包數據集為 `.zip` 檔**下載。
    * **🌐 網路與 Tailscale 設定**:
      * 查看樹莓派當前的網卡 IP 以及 **Tailscale 固定虛擬網 IP**。
      * 支援直接於網頁修改並保存 PC 推論伺服器 IP (寫入 `.env`)。
      * 允許直接登錄新的 Wi-Fi 連線設定（自動切換 SSID），實現出門 Demo 流程的完全自動化。
    * **⚙️ 舵機與重量硬體調校**:
      * **雙軸舵機手動測試**: 支援點選「啟用實體序列埠」，手動調整 Pitch 與 Yaw 舵機角度以測試機械動作是否到位。
      * **重量感測器實時校準**: 實時監測 HX711 原始 ADC 值與重量，並可一鍵執行去皮 (Tare) 歸零或寫入校準值。
